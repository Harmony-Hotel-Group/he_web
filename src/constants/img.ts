...
const GLOB_FLAGS = import.meta.glob<{ default: ImageMetadata }>(
	"/src/assets/img/flags/*.{svg,png,jpg,jpeg,webp}",
	{ eager: true },
);
const GLOB_CURRENTS = import.meta.glob<{ default: ImageMetadata }>(
	"/src/assets/img/currents/*.{svg,png,jpg,jpeg,webp}",
	{ eager: true },
);
...
const GLOB_HOTEL = import.meta.glob<{ default: ImageMetadata }>(
	"/src/assets/img/hotel/**/*.{svg,png,jpg,jpeg,webp}",
	{ eager: true },
);
const GLOB_CITIES = import.meta.glob<{ default: ImageMetadata }>(
	"/src/assets/img/cities/**/*.{svg,png,jpg,jpeg,webp}",
	{ eager: true },
);
...
function formatKeys(minimized: boolean = true) {
	return (
		acc: Record<string, ImageMetadata>,
		[path, module]: [string, { default: ImageMetadata }],
	) => {
...
export const FLAGS = Object.entries(GLOB_FLAGS).reduce(
	formatKeys(),
	{} as Record<string, ImageMetadata>,
);
export const CURRENTS = Object.entries(GLOB_CURRENTS).reduce(
	formatKeys(),
	{} as Record<string, ImageMetadata>,
);
...
export const HOTEL = Object.entries(GLOB_HOTEL).reduce(
	formatNestedKeys("/src/assets/img/hotel/"),
	{} as Record<string, ImageMetadata>,
);
export const CITIES = Object.entries(GLOB_CITIES).reduce(
	formatNestedKeys("/src/assets/img/cities/"),
	{} as Record<string, ImageMetadata>,
);
