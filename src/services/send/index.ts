 ...
 */
export async function sendBookingMessage(formData: FormData, type: BookingType): Promise<void> {
    try {
 ...
 */
async function processBookingData(formData: FormData, type: BookingType): Promise<ProcessedBookingData> {
 ...
 */
function formatBookingMessage(data: ProcessedBookingData, type: BookingType): string {
 ...
 * @param type - Tipo de reserva
 */
export async function sendEmail(data: ProcessedBookingData, type: BookingType): Promise<void> {
    try {
 ...
 */
function generateEmailTemplate(data: ProcessedBookingData, type: BookingType): string {
 ...
 */
function getEmailContent(data: ProcessedBookingData, type: BookingType): string {
    switch (type) {
        case 'normal':
            return `
 ...
 */
function getVehiclesEmailTable(vehicles: Array<{type: string, plate: string}>): string {
    return `
        <h3>Vehículos</h3>
        <table>
            <thead>
 ...
 */
function generateTextTemplate(data: ProcessedBookingData, type: BookingType): string {
    const currentDate = new Date().toLocaleDateString('es-ES');
 ...
 */
function getVehiclesTextList(vehicles: Array<{type: string, plate: string}>): string {
 ...
 */
function getVehiclesData(formData: FormData, count: number): Array<{type: string, plate: string}> {
    const vehicles = [];
 ...
 */
export function validateBookingData(formData: FormData, type: BookingType): string[] {
    const errors: string[] = [];
