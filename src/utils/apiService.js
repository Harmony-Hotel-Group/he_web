...
let lastApiDataCache = {}; // Cache for the last successfully fetched API data
let lastDataUpdateTime = {};
const STATUS_CHECK_INTERVAL = parseInt(import.meta.env.STATUS_CHECK_INTERVAL_MINUTES || '5') * 60 * 1000;
...
async function sendAdminNotification(subject, text) {
    ...
    if (!MAILGUN_API_KEY || !ADMIN_EMAIL || !SENDER_EMAIL || !MAILGUN_DOMAIN) {
        console.warn('Mailgun API key, Admin Email, Sender Email or Mailgun Domain not configured. Skipping notification.');
        return;
    }
        ...
        if (now - (lastDataUpdateTime[dataType] || 0) > DATA_UPDATE_INTERVAL) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.warn(`Failed to fetch ${dataType} from API (${response.status}). Falling back to cached API data or local data.`);
                ...
                console.error(`Error fetching ${dataType} from API. Falling back to cached API data or local data.`, error);
                // Si la petición a la API falla, intenta devolver la última data de la API en caché, si no, la data local
                return lastApiDataCache[dataType] || await getLocalData(dataType);
            }
        } else {
            ...
            console.log(`Using cached API data for ${dataType} (within update interval).`);
            return lastApiDataCache[dataType] || await getLocalData(dataType);
        }
    } else {
