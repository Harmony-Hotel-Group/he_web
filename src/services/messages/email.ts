...
const ENV = {
  ...
  SENDER_EMAIL: (import.meta.env.SENDER_EMAIL as string | undefined) ?? 'no-reply@yourdomain.com',
...
async function doMailgunSend(params: URLSearchParams) {
  if (!ENV.MAILGUN_API_KEY || !ENV.MAILGUN_DOMAIN) {
    if (ENV.DEV) console.warn('[messages/email] Mailgun not configured, skipping send');
...
export async function sendAdminEmail(subject: string, text?: string, html?: string) {
  if (!ENV.ADMIN_EMAIL) {
    if (ENV.DEV) console.warn('[messages/email] ADMIN_EMAIL not set, skipping admin email');
    return { ok: false, skipped: true } as const;
  }
