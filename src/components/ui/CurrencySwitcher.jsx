...
export default function CurrencySwitcher() {
  ...
  return (
    <div class="relative currency-switcher-container">
      ...
      >
        <img src={selectedCurrencyData.image} alt={`${selectedCurrencyData.code} symbol`} class="h-5 w-5" /> {/* Only show the image */}
        {/* Text and arrow are now hidden by default */}
        <span class="hidden">{selectedCurrencyData.code}</span>
        <svg class="w-3 h-3 ml-1 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
      </button>
      ...
      {isOpen && (
        <div class="absolute right-0 mt-2 w-24 bg-gray-700 rounded-md shadow-lg z-20">
          {Object.entries(currencies).map(([currencyCode, data]) => (
            ...
            >
              <img src={data.image} alt={`${data.code} symbol`} class="h-4 w-4" />
              <span>{data.code}</span>
            </button>
          ))}
        </div>
