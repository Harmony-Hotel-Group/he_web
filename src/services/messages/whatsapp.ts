...
export async function sendWhatsappMessage(toE164: string, bodyText: string, options: WhatsAppOptions = {}) {
  const token = options.token ?? ENV.WHATSAPP_TOKEN;
  const phoneId = options.phoneId ?? ENV.WHATSAPP_PHONE_ID;
  if (!token || !phoneId) {
    if (ENV.DEV) console.warn('[messages/whatsapp] Missing WHATSAPP_TOKEN or WHATSAPP_PHONE_ID; skipping');
